...
// Read the raw token from the environment variable (digital_signage:XXXX...)
...
// ==========================================================
// 🚨 TEMPORARY DEBUGGING ROUTE (Confirms key status) 🚨
// ==========================================================
app.get('/', (req, res) => {
...
app.get('/api/get-yodeck-media-data', async (req, res) => {
    ...
    try {
        // 🛑 CRITICAL LINE: Uses the RAW_TOKEN_VALUE, which contains the prefix.
        const apiResponse = await fetch(MEDIA_LIST_ENDPOINT, {
            method: 'GET',
            headers: {
                'Authorization': `Token ${API_LABEL}:${RAW_TOKEN_VALUE}`, 
        ...
        if (!apiResponse.ok) {
            ...
            } catch (e) { /* Ignore failure to read error body */ }
